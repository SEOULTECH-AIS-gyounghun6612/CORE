"""카메라 모델 및 투영 연산 functional 모듈."""

import numpy as np
from vision_toolbox.typing.images import (
    V_1D, V_2D, V_3D, V_SIZE, M_IN, IMG_1C
)
...
def Get_focal_length_from_fov(fov: V_2D, sz: V_SIZE) -> V_2D:
    """시야각(FOV)과 이미지 크기로부터 초점 거리를 계산합니다."""
    return sz / (2 * np.tan(np.deg2rad(fov) / 2))


def Get_size_from_fov(fov: V_2D, f_len: V_2D) -> V_SIZE:
    """시야각(FOV)과 초점 거리로부터 이미지 크기를 계산합니다."""
    _rads = np.deg2rad(fov) / 2
    _size = 2 * np.round(np.tan(_rads) * f_len)
    return _size.astype(np.int32)


def Get_fov_from_size(sz: V_SIZE, f_len: V_2D) -> V_2D:
    """이미지 크기와 초점 거리로부터 시야각(FOV)을 계산합니다."""
    return 2 * np.rad2deg(np.arctan2(sz / 2, f_len))


def Get_principal_point(rate: V_2D, sz: V_SIZE) -> V_2D:
    """비율과 이미지 크기로부터 주점을 계산합니다."""
    return sz * rate


def Get_principal_point_rate(pp: V_2D, sz: V_SIZE) -> V_2D:
    """주점과 이미지 크기로부터 주점의 비율을 계산합니다."""
    return pp / sz


def Compose_intrinsic_matrix(f_len: V_2D, pp: V_2D) -> M_IN:
...
def Extract_intrinsic_params(in_m: M_IN) -> tuple[V_2D, V_2D]:
...
def Adjust_intrinsic_matrix(in_m: M_IN, sz: V_SIZE, new_sz: V_SIZE) -> M_IN:
...
def Apply_intrinsic_transform(
    pts: V_3D, in_m: M_IN, inv: bool = False
) -> V_3D:
...
def Get_points_from_depth(d_map: IMG_1C, mask: IMG_1C | None = None) -> V_3D:
...
def Get_depth_map_from_points(pts: V_3D, d_sz: tuple[int, int]) -> IMG_1C:
...
def Remap_depth_map(depth: IMG_1C, in_m: M_IN, new_sz: tuple[int, int]) -> IMG_1C:
