...
- **방위는 4-fold 로 고정한다.** 주축은 선이라 90° 주기로 모호하다 — 최대 질량 사분면이 BR
  (우·하)에 오도록 회전을 골라 방위를 하나로 굳힌다(근대칭 형상도). u 가 major 라는 보장이
  없어지므로 major/minor 크기는 축 이름이 아니라 고유값 크기순으로 뽑는다(`Centroid_Frame`
  ↔ `Region_Scalars` 한 벌).
