...
# if this file in package folder
from ._torch_base import Process_Name, System_Utils, Tracking, Scale_Type
from ._label import Label
from ._dataset import Custom_Dataset, Augment
from ._layer import Custom_Model
from ._optimizer import _LRScheduler, Suport_Optimizer, Suport_Schedule, _Optimizer_build
...
class Basement():
    ...
    # Freeze function
    # --- for initialize --- #
    def _Set_learning_option(self, max_epoch: int, last_epoch: int, mode_list: List[Process_Name]):
        """
        Parameters
        --------------------
        """
        ...
        print(f"{self._project_name}")

    def _Set_multi_process_option(
        self,
        world_size: int = 1,
        this_rank: int = 0,
    ...
    def _Set_dataloader(
            self,
            dataset: Type[Custom_Dataset],
            label_process: Label.Process.Basement,
            data_process: Label.File_IO.Basement,
            amplification: Dict[Process_Name, int],
            augmentation: Dict[Process_Name, Union[Augment.Basement, List[Augment.Basement]]],
            batch_size_per_node: int,
            num_worker_per_node: int,
    ):
        self._dataset: Dict[Process_Name, Custom_Dataset] = {}
        for _mode, _amp_size in amplification.items():
            self._dataset.update({
                _mode: dataset(label_process, data_process._Get_file_profiles(_mode), _amp_size, augmentation[_mode])
            })
    ...
    def _Set_model_option(self, model_structure: Type[Custom_Model], **model_parameter):
        self._model_structure = model_structure
        self._model_parameters = model_parameter

    def _Set_optim_option(self, optim_name: Suport_Optimizer, initial_lr: float, schedule_name: Suport_Schedule, **schedule_parmeter):
    ...
    def _Set_Tracker(self, Learning_info: Dict, logging_param: Dict[Process_Name, str]):
        self._tracker = System_Utils.Learning_Logger(Learning_info, logging_param)
    ...
    # Un-Freeze function
    # --- for process --- #
    def _Apply_active_mode(self, mode: Process_Name):
...
class End_to_End():  # In later, re-build this class from Basement code using to inheritance
    def __init__(
        self,
        max_epoch: int,
        learning_mode: List[Process_Name],
        batch_size_per_node: int,
        num_worker_per_node: int,
        last_epoch: int = -1,
        ...
        if len(gpu_ids) > 1 and distributed.is_nccl_available():
            ...
            # - use only one gpu
            self._gpu_list = gpu_ids
            self._multi_method = Multi_Method.NOT_USE
            self._world_size = 1
        else:
            # - not use gpu but can use multi process
            self._gpu_list = []
            self._multi_method = Multi_Method.NOT_USE if multi_method is Multi_Method.DDP else multi_method
            self._world_size = 1 if self._multi_method is Multi_Method.NOT_USE else world_size
    ...
    def _Set_tracker(
        self,
        tracking_param: Dict[Process_Name, Dict[Scale_Type, List[str]]],
        observing_param: Dict[Process_Name, Dict[Scale_Type, Optional[List[str]]]]
    ):
    ...
    def _Active_mode_change_to(
            ...
            sampler: Dict[Process_Name, Optional[DistributedSampler]]):
    ...
    def _Process(self, processer_num: int, share_block: Optional[multiprocessing.Queue] = None):
        ...
        _dataloader: Dict[Process_Name, DataLoader] = {}
        _sampler: Dict[Process_Name, Optional[DistributedSampler]] = {}
        ...
        if self._multi_method == Multi_Method.DDP:
            ...
            for _this_mode in self._learning_mode:
                _sampler[_this_mode] = DistributedSampler(self._dataset[_this_mode], rank=_this_node, shuffle=(_this_mode == Process_Name.TRAIN))
        ...
        # - Not use multi-process
            ...
            for _this_mode in self._learning_mode:
                ...
                _dataloader[_this_mode] = DataLoader(
                    ...
                    batch_size=self._batch_size,
                    num_workers=self._num_worker,
                    shuffle=_this_mode == Process_Name.TRAIN)
            # Set model optim and scheduler
            _model, _optim, _scheduler = self._Set_learning_model(_this_gpu_id)
        ...
        for _epoch in range(self._last_epoch + 1, self._max_epoch):
            ...
            for _this_mode in self._learning_mode:
                ...
                if _this_mode == Process_Name.TRAIN:
...
class Reinforcement():
    class Deep_Q_Network(End_to_End):
        def __init__(
                self,
                max_epoch: int,
                learning_mode: List[Process_Name],
        ...
        def _Process(self, processer_num: int, share_block: Optional[multiprocessing.Queue] = None):
            return super()._Process(processer_num, share_block)

    class Acter_Critic(Basement):
        # Freeze function
        # --- for initialize --- #
        def _Set_reinforcement_option(self, ):
            ...

        def _Set_reword_model(self):
            ...
