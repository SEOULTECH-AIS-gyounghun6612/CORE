...
class Gaussian_Model:
    def __init__(self, sh_degree: int, config=None):
        ...
        self.rotation_activation = functional.normalize
    ...
    def extend_from_pcd(
        ...
        new_xyz = Parameter(fused_point_cloud.requires_grad_(True))
        new_features_dc = Parameter(
            features[:, :, 0:1].transpose(1, 2).contiguous().requires_grad_(True)
        )
        new_features_rest = Parameter(
            features[:, :, 1:].transpose(1, 2).contiguous().requires_grad_(True)
        )
        new_scaling = Parameter(scales.requires_grad_(True))
        new_rotation = Parameter(rots.requires_grad_(True))
        new_opacity = Parameter(opacities.requires_grad_(True))
    ...
    def load_ply(self, path):
        ...
        self._opacity = Parameter(
            torch.tensor(opacities, dtype=torch.float, device="cuda").requires_grad_(
                True
            )
        )
        self._scaling = Parameter(
            torch.tensor(scales, dtype=torch.float, device="cuda").requires_grad_(True)
        )
        self._rotation = Parameter(
    ...
    def replace_tensor_to_optimizer(self, tensor, name):
        ...
        for group in self.optimizer.param_groups:
            if group["name"] == name:
                ...
                group["params"][0] = Parameter(tensor.requires_grad_(True))
